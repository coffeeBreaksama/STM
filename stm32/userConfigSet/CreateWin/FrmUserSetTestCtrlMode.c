...
static const char* prStringArray[][3] ={
	...
	{"下","下","DOWN"},
	{"下一步","下一步","NEXT"},
	{"等速位移","等速位移","Disp Speed Keeping"},
	{"力量保持(堆码)","力量保持(堆码)","Load keeping"},
	{"疲劳","疲劳","Fatigue test"},
	NULL
} ;
...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  switch (pMsg->MsgId) {
	  case WM_INIT_DIALOG:
		...
		LISTBOX_SetSel(hItem,__configDataU.ctrlMethod);
		break;
...
static WM_HWIN TestModuleSet(void) {
...
{
	__WindowsInfo[UI_GetWindowInfoIndexId(FORMID_TESTMODULE)].FrmHandle = TestModuleSet();
	return 0xFF;
}
