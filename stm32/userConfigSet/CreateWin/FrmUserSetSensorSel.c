...
static const char* prStringArray[][3] ={
	{"设置/荷重元选择","设置/荷重元选择","SensorSelect"},
	{"","",""},
	{"保存","保存","SAVE"},
	{"取消","取消","CANCEL"},
	{"上","上","UP"},
...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  switch (pMsg->MsgId) {
	  case WM_INIT_DIALOG:
		...
		LISTBOX_SetSel(WM_GetDialogItem(pMsg->hWin, ID_LISTBOX_0),__configDataU.whichLoadCell); 
	  break;
	...
	case ID_BUTTON_SAVE: // Notifications sent by 'Save'
      switch(NCode) {
		case WM_NOTIFICATION_RELEASED:
			listSel = LISTBOX_GetSel(WM_GetDialogItem(pMsg->hWin, ID_LISTBOX_0));
			User_Set_whichLoadCell(listSel);
			sprintf(strBuffer,"%d",__configDataU.whichLoadCell);
...
*/
static WM_HWIN SensorSelect(void) {
...
unsigned int RunFrmUserSetSensorSel(void)
{
	...
	__WindowsInfo[UI_GetWindowInfoIndexId(FORMID_SENSORSELECT)].FrmHandle = SensorSelect();
	return 0xFF;
