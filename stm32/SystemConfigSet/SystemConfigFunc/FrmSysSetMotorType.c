...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  case WM_INIT_DIALOG:
    ...
    LISTBOX_SetSel(hItem,__configDataS.motorType);
...
unsigned int RunFrmSysSetMotorType(void)
{
    ...
    __WindowsInfo[UI_GetWindowInfoIndexId(FORMID_MOTORTYPE)].FrmHandle = MotorType();
    return 0xFF;
}
